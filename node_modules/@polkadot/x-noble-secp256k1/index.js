...
export { CURVE }; // y² = x³ + ax + b: Short weistrass curve formula. Returns y²
...
const USE_ENDOMORPHISM = CURVE.a === _0n; // Default Point works in 2d / affine coordinates: (x, y)
...
const POW_2_128 = _2n ** _128n; // Split 256-bit K into 2 128-bit (k1, k2) for which k1 + k2 * lambda = K.
...
async function getQRSrfc6979(msgHash, privateKey) {
  ...
  const hmac = utils.hmacSha256; // Steps D, E, F, G

  k = await hmac(k, v, b0, x, h1);
  v = await hmac(k, v);
  k = await hmac(k, v, b1, x, h1);
  v = await hmac(k, v); // Step H3, repeat until 1 < T < n - 1
...
export { sign, signSync }; // https://www.secg.org/sec1-v2.pdf, section 4.1.4
...
async function schnorrSign(msgHash, privateKey, auxRand = utils.randomBytes()) {
  if (msgHash == null) throw new TypeError(`sign: Expected valid message, not "${msgHash}"`); // if (privateKey == null) throw new TypeError('Expected valid private key');
  ...
  const k0 = mod(k0h, n);
  if (k0 === _0n) throw new Error('sign: Creation of signature failed. k is zero'); // R = k'⋅G
...
async function schnorrVerify(signature, msgHash, publicKey) {
  ...
  const e = await createChallenge(sig.r, P, m); // R = s⋅G - e⋅P
