...
function scryptInit(password, salt, _opts) {
  ...
  (0, _utils.assertNumber)(N);
  (0, _utils.assertNumber)(r);
  (0, _utils.assertNumber)(p);
  (0, _utils.assertNumber)(dkLen);
  (0, _utils.assertNumber)(asyncTick);
  (0, _utils.assertNumber)(maxmem);
  ...
  const B = (0, _pbkdf.pbkdf2)(_sha.sha256, password, salt, {
    c: 1,
    dkLen: blockSize * p
  });
  const B32 = (0, _utils.u32)(B); // Re-used between parallel iterations. Array(iterations) of B

  const V = (0, _utils.u32)(new Uint8Array(blockSize * N));
  const tmp = (0, _utils.u32)(new Uint8Array(blockSize));
...
function scryptOutput(password, dkLen, B, V, tmp) {
  const res = (0, _pbkdf.pbkdf2)(_sha.sha256, password, B, {
    c: 1,
    dkLen
  });
  B.fill(0);
...
async function scryptAsync(password, salt, _opts) {
  ...
  for (let pi = 0; pi < p; pi++) {
    ...
    let pos = 0;
    await (0, _utils.asyncLoop)(N - 1, asyncTick, i => {
      BlockMix(V, pos, V, pos += blockSize32, r); // V[i] = BlockMix(V[i-1]);
    ...
    blockMixCb();
    await (0, _utils.asyncLoop)(N, asyncTick, i => {
      // First u32 of the last 64-byte block (u32 is LE)
      const j = B32[Pi + blockSize32 - 16] % N; // j = Integrify(X) % iterations
