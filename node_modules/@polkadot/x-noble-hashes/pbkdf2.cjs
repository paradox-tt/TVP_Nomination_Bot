...
function pbkdf2Init(hash, _password, _salt, _opts) {
  ...
  (0, _utils.assertNumber)(c);
  (0, _utils.assertNumber)(dkLen);
  (0, _utils.assertNumber)(asyncTick);
  if (c < 1) throw new Error('PBKDF2: iterations (c) should be >= 1');
  const password = (0, _utils.toBytes)(_password);
  const salt = (0, _utils.toBytes)(_salt); // DK = PBKDF2(PRF, Password, Salt, c, dkLen);

  const DK = new Uint8Array(dkLen); // U1 = PRF(Password, Salt + INT_32_BE(i))

  const PRF = _hmac.hmac.init(hash, password);
...
function pbkdf2(hash, password, salt, _opts) {
  ...
  const view = (0, _utils.createView)(arr);
...
async function pbkdf2Async(hash, password, salt, _opts) {
  ...
  for (let ti = 1, pos = 0; pos < dkLen; ti++, pos += PRF.outputLen) {
    ...
    Ti.set(u.subarray(0, Ti.length));
    await (0, _utils.asyncLoop)(c - 1, asyncTick, i => {
      // Uc = PRF(Password, Uc−1)
      PRF._cloneInto(prfW).update(u).digestInto(u);
