...
import { Keccak } from "./sha3.js"; // cSHAKE && KMAC (NIST SP800-185)
...
const toBytesOptional = buf => buf !== undefined ? toBytes(buf) : new Uint8Array([]); // NOTE: second modulo is necessary since we don't need to add padding if current element takes whole block
...
class KMAC extends Keccak {
  constructor(blockLen, outputLen, enableXOF, key, opts = {}) {
    ...
    key = toBytes(key); // 1. newX = bytepad(encode_string(K), 168) || X || right_encode(L).
...
export const kmac256xof = genKmac(136, 256 / 8, true); // TupleHash
...
export const tuplehash256xof = genTuple(136, 256 / 8, true); // ParallelHash (same as K12/M14, but without speedup for inputs less 8kb, reduced number of rounds and more simple)
...
function genParallel(blockLen, outputLen, leaf, xof = false) {
  ...
  parallel.create = (opts = {}) => new ParallelHash(blockLen, opts.dkLen !== undefined ? opts.dkLen : outputLen, () => leaf.init({
    dkLen: 2 * outputLen
  }), xof, opts);
...
export const parallelhash256xof = genParallel(136, 256 / 8, cshake256, true); // Kangaroo
...
export const k12 = wrapConstructorWithOpts((opts = {}) => new KangarooTwelve(168, 32, opts.dkLen !== undefined ? opts.dkLen : 32, 12, opts)); // MarsupilamiFourteen

export const m14 = wrapConstructorWithOpts((opts = {}) => new KangarooTwelve(136, 64, opts.dkLen !== undefined ? opts.dkLen : 64, 14, opts)); // https://keccak.team/files/CSF-0.1.pdf
// + https://github.com/XKCP/XKCP/tree/master/lib/high/Keccak/PRG

class KeccakPRG extends Keccak {
  constructor(capacity) {
    assertNumber(capacity); // Rho should be full bytes
