...
var blake2 = _interopRequireWildcard(require("./_blake2.cjs"));

var blake2s = _interopRequireWildcard(require("./blake2s.cjs"));
...
class BLAKE3 extends blake2.BLAKE2 {
  ...
  constructor() {
    ...
    this.bufferOut = void 0;
    this.chunkOut = 0;
    this.enableXOF = true;
    this.outputLen = opts.dkLen === undefined ? 32 : opts.dkLen;
    (0, _utils.assertNumber)(this.outputLen);
    ...
    this.bufferOut = (0, _utils.u8)(this.bufferOut32);
  } // Unused
  ...
  b2Compress(counter, flags, buf) {
    let bufPos = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 0;
    const {
      state,
      pos
    } = this;
    const {
      h,
      l
    } = u64.fromBig(BigInt(counter), true); // prettier-ignore
    ...
    const {
      ...
      v14,
      v15
    } = blake2s.compress(SIGMA, bufPos, buf, 7, state[0], state[1], state[2], state[3], state[4], state[5], state[6], state[7], blake2s.IV[0], blake2s.IV[1], blake2s.IV[2], blake2s.IV[3], h, l, pos, flags);
    state[0] = v0 ^ v8;
    state[1] = v1 ^ v9;
    state[2] = v2 ^ v10;
    state[3] = v3 ^ v11;
  ...
  compress(buf) {
    let bufPos = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
    ...
    this.chunkPos += 1; // If current block is last in chunk (16 blocks), then compress chunks

    if (this.chunkPos === 16 || isLast) {
      let chunk = this.state;
      this.state = this.IV.slice(); // If not the last one, compress only when there are trailing zeros in chunk counter
  ...
  b2CompressOut() {
    ...
    const {
      h,
      l
    } = u64.fromBig(BigInt(this.chunkOut++)); // prettier-ignore
    ...
    const {
      ...
      v14,
      v15
    } = blake2s.compress(SIGMA, 0, buffer32, 7, state[0], state[1], state[2], state[3], state[4], state[5], state[6], state[7], blake2s.IV[0], blake2s.IV[1], blake2s.IV[2], blake2s.IV[3], l, h, pos, flags);
    bufferOut32[0] = v0 ^ v8;
    bufferOut32[1] = v1 ^ v9;
  ...
  finish() {
    ...
    this.buffer.fill(0, this.pos); // Process last chunk
...
const blake3 = (0, _utils.wrapConstructorWithOpts)(opts => new BLAKE3(opts));
